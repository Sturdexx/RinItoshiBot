...
import RinItoshi.modules.sql.feds_sql as sql
...
from RinItoshi.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
...
from RinItoshi.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "RinItoshi_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="RinItoshi_fbanned_users.json",
        ...
        if args[0] == "csv":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "RinItoshi_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="RinItoshi_fbanned_users.csv",
